#!/usr/bin/env python3
"""List available agent commands from configured remote repositories.
...
def main():
    ...
    for issue_url in urls:
        ...
        try:
            ...
            for agent in repos.get("agents", []):
                name = agent.get("name", "")
                if name and name not in seen:
                    seen.add(name)
                    agent_repos.append(agent)
        except Exception as e:
            print(f"Warning: Failed to fetch {issue_url}: {e}")

    if not agent_repos:
        print("No agent repositories configured.")
        sys.exit(0)
    ...
    for repo_info in agent_repos:
        ...
        path = repo_info.get("path", "agent-commands")
        ...
        try:
            ...
            for item in contents:
                ...
                if "template" in name.lower():
                    continue
                if item["type"] == "file" and name.endswith(".md"):
                    agents.append(name.rsplit(".", 1)[0])
                elif item["type"] == "dir":
                    agents.append(name)

            for a in sorted(agents):
                print(f"  - {a}")
            print(f"  Total: {len(agents)} agent commands available")
            total += len(agents)
    ...
    print(f"\nGrand total: {total} agent commands across {len(agent_repos)} repositories")
